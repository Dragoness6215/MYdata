...
function HomeScreen({ navigation }) {
  ...
  const switchState=()=>{
    updateStateSwitch(previousState => !previousState);
  }
          ...
          <View style={styles.bottomLine}></View>
            <View>
              <TouchableWithoutFeedback onPress={() => navigation.navigate('All Graphs')}>
                  <Text style={styles.bigButton}>Graphs</Text>
              </TouchableWithoutFeedback>
            </View>
            <View>
              <TouchableWithoutFeedback onPress={() => checkAndNavigate()}>
                  <Text style={styles.bigButton}>Connect </Text>
              </TouchableWithoutFeedback>
            </View>
            {/*<View>
            ...
            <View>
              <TouchableWithoutFeedback onPress={() => navigation.navigate('About')}>
                  <Text style={styles.bigButton}>About</Text>
              </TouchableWithoutFeedback>
            </View>
          </View>
    );
  ...
  return (
    <SafeAreaView style= {styles.container}>
      ...
      {activeComponent === 'List' ? (
              ...
              <View style={styles.bottomLine}></View>
                <FlatList name="List"  data={textArray}
                    renderItem={({item}) => 
  ...
  <SafeAreaView style= {styles.container}>
        ...
        <View>
          ...
          <Picker
            ...
            <Picker.Item label="Pick One" value="null" style={styles.pickerDropdown}></Picker.Item>
            {loadTypes()}
          </Picker>
        </View>
        <View>
...
function SelectData({ navigation }) {
  ...
  const updateData = async(toDo) =>{
    if(toDo=="remove"){
      await AsyncCode.removeItem(tempkey);
    ...
    await AsyncCode.restoreFromAsync();
    setThisState({
      showAlert:false,
      textArray:AsyncCode.getTextArray(),
    });
  }
      ...
      <ScrollView>
        <View style = {styles.container}>
          <View>
              ...
              <View>
                <FlatList
                    data={thisState.textArray}
                    renderItem={({ item }) =><View style={styles.bottomLine}>
              ...
              <View style={styles.buttonBox}>
                  ...
                  <View>
                      ...
                      <Text style={styles.smallButton}> Add New Graph </Text>
                      </TouchableWithoutFeedback>
                  </View>
              </View>
            </View>
...
function Graph({ navigation}) {
    ...
    const checkIfAsyncChanged = async() => {
      ...
      if(GLOBAL.BUTTONPRESSED){
        ...
        let temp1 = await AsyncCode.getTextArray();
        const temp2 = temp1.filter(textArray => textArray.Key == key);
        await setRawData(temp2[0]);
        checking=false;
    ...
    return (
      <SafeAreaView style= {styles.container}>
        ...
        <View>
          ...
          <Text style={styles.subheader}> Add Data </Text>
            ...
            <TouchableWithoutFeedback onPress={() => {navigateAndSendDataAlter(RawData)}}>
              <Text style={styles.bigBlueButton}>{"Change Data"}</Text>
            </TouchableWithoutFeedback>
        </View>
          <AwesomeAlert
                show={showAlert}
...
function AlterData({ navigation}) {
  ...
  const setDesctiption = async (text) =>{
    ...
    setRawData(tempData);
    await AsyncCode.alterAsyncItem(tempData);
  ...
  const setTitle = async (text) =>{
    let newName=true;
      ...
      let rawData=await AsyncCode.getTextArray();
  ...
  const addDataPointDescription= async(item,description) => {
    ...
    if(description.length>=1){
      ...
      setShowError(false);
      let temp = await AsyncCode.changeDataPointDescription(RawData.Key,item,description);
      setCurData(temp.Data);
      setFilteredData(temp.Data);
      GLOBAL.ITEM=temp;
      throwSuccessAlert(true);
  ...
  return (
    <SafeAreaView style= {styles.container}>
      <ScrollView style = {styles.scrollView}>
        ...
        <Text style={styles.title}> {"Change "+ RawData.Title} </Text>
        <Picker
              style={styles.picker}
              selectedValue={whatToShow}
              mode="dropdown"
              onValueChange={(itemValue, itemIndex) => (setShowing(itemValue))
            }>
